...
$(function(){
	...
	// 비밀번호 찾기를 눌렀을 때
	$("#goFindPass").on('click',function(){
		location.href = "<%=request.getContextPath()%>/view/login/findPass.jsp"
	});
	...
	$('#findIdSub').on('click', function(){
		...
		if(name.length == 0 || mail.length == 0){
			$('#idFindForm').addClass('was-validated');
			return false;
		}
...
<section>
	...
	<div class="row">
		...
		<h2 class="logintext">아이디 찾기</h2>
		<div>
		
			<br><br>
			<form action="<%=request.getContextPath()%>/meberFindId.do" class="needs-validation" method="post" id="idFindForm" novalidate>
			<input type="hidden" name = "job" value = "findId">
				...
				<div class="form-group">
					...
					<br>
					<button type="button" class="btn btn-primary btn-sm checkId" id="findIdSub">아이디 찾기</button>
				</div>
					
				<button type="button" class="btn btn-primary btn1" id="goLogin">뒤로가기</button>
				<button type="button" class="btn btn-primary btn2" id = "goFindPass">비밀번호 찾기</button>	
			</form>
		</div>
	</div>
	</div>
